...
4. Use the grid record/hold controls to start and stop capture for a mapped target track.
...
- The script monitors note on/off and CC input, but loop recording control is grid-driven.
...
- This starter keeps the current engine behavior from `midididi`: single device and absolute CC values.
- Note on/off messages do not start or stop recording; recording control is handled by grid hold/release actions.
